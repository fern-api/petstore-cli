...
[![crates.io shield](https://img.shields.io/crates/v/petstore_api_sdk)](https://crates.io/crates/petstore_api_sdk)

The PetStore Rust library provides convenient access to the PetStore APIs from Rust.
...
```toml
[dependencies]
petstore_api_sdk = "0.1.0"
```

Or install via cargo:

```sh
cargo add petstore_api_sdk
```
...
```rust
match client.pets.create_pet(None)?.await {
    Ok(response) => {
...
```rust
let response = client.pets.create_pet(
