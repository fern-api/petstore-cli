...
impl petstore_api_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn sdk_client(ctx: &AppContext) -> petstore_api_sdk::api::ApiClient {
    let executor = ctx.build_sdk_executor();
    let adapter = Arc::new(CliExecutorAdapter(executor));
    let config = petstore_api_sdk::ClientConfig::default();
    let http_client = petstore_api_sdk::HttpClient::with_executor(
        adapter as Arc<dyn petstore_api_sdk::RequestExecutor>,
        config.clone(),
    );
    petstore_api_sdk::api::ApiClient {
        config,
        pets: petstore_api_sdk::api::PetsClient { http_client: http_client.clone() },
        auth: petstore_api_sdk::api::AuthClient { http_client: http_client.clone() },
    }
}
...
where
    F: Future<Output = Result<T, petstore_api_sdk::ApiError>>,
{
...
fn convert_api_error(e: petstore_api_sdk::ApiError) -> CliError {
    match e {
        petstore_api_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        petstore_api_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        // Errors raised by the CLI's injected executor arrive boxed. Recover
        // the concrete `SdkError` so structured failures (auth, timeout, …)
        // map to the right `CliError` variant instead of a flat string.
        petstore_api_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
